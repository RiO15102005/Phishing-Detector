...
class DNSResolver:
    ...
    def resolve_ipv4(self, hostname: str) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"IPv4 Resolve Failed : {hostname} : {ex}")
    ...
    def resolve_ipv6(self, hostname: str) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"IPv6 Resolve Failed : {hostname} : {ex}")
    ...
    def collect(self, url: str) -> Dict:
        logger.info(f"Resolve DNS : {url}")

        hostname = self.extract_hostname(url)

        domain = self.extract_domain(hostname)

        ipv4 = self.resolve_ipv4(hostname)

        ipv6 = self.resolve_ipv6(hostname)
        ...
        logger.info(f"IPv4 : {len(ipv4)}")

        logger.info(f"IPv6 : {len(ipv6)}")

        logger.info("DNS Resolver Finished")
